...
data of an organism combined from an 'org' package (e.g., org.Hs.eg.db) and the
...
The `src_organism()` constructor creates an on disk sqlite database with data 
from given 'TxDb' package and corresponding 'org' package. When dbpath is
given, file is created at the given path, otherwise temporary file is created.
...
database, if genome and id are not given, most recent 'TxDb' package is used.
